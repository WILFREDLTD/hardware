...
export function ProductFormModal({ isEditing, isSaving, formData, categories, baseUnits, suppliers, onClose, onSubmit, onChange, onCategoryChange, onBaseUnitChange, onSupplierChange }: Props) {
  ...
  return (
    <Modal title={isEditing ? 'Edit Product' : 'Add Product'} onClose={onClose} onSubmit={onSubmit} submitDisabled={isSaving} submitLabel={isSaving ? 'Saving...' : undefined}>
      <div className="space-y-4">
        ...
        <div className="flex flex-col">
          <label className="text-sm font-medium text-gray-700 mb-1.5">Supplier</label>
          <select value={formData.supplierId} onChange={onSupplierChange} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-green-500/30 focus:border-green-500 transition-all">
            ...
            <option value="NEW_SUPPLIER" className="font-semibold text-blue-600">+ New Supplier</option>
          </select>
        </div>
      </div>
    </Modal>
